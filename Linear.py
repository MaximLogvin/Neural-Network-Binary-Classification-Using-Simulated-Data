...
def p(x,points):
    for i in range(len(points)-1):
        if (points[i][0]<=x and x<=points[i+1][0]):
            return points[i][1]+(points[i+1][1]-points[i][1])/(points[i+1][0]-points[i][0])*(x-points[i][0])
...
while(val<0.95):
    ...
    odd+=1
    if(odd%2==1):
        points.append([val,0.7])
    else:
        points.append([val,0.3])
points.append([1,1])
...
# for i in range(11):
#     if(i%2==0):
#         plt.annotate(f"({points[i][0]},{points[i][1]})",(points[i][0],points[i][1]),textcoords="offset points",xytext=(0,-20),ha="center")
#     else:
